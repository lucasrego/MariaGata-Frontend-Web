...
$token_egoi = "340e2e65380dbad2002f4ca4205ddd3740fb950a";
$listaEgoi = 3;
...
*/
try {
	include_once 'egoi.php';
	$egoi = new Egoi($token_egoi);
	//$egoi->add_tag(7,"lucasrego@gmail.com",13);
	$dados = $egoi->subscriber_data($listaEgoi, $email);
	$uid = $dados["Egoi_Api"]["subscriberData"]["subscriber"]["UID"];
} catch (Exception $e) {
	$uid = "";
}
...
$seven_bit_msg = "Oi " . $nome . "! Seu Voucher " . $uid . "-v foi gerado e ja pode ser usado. Agende agora seu momento pelo WhatsApp 71 98879-1014. Nos vemos na Maria Gata! Juliana";
...
function send_message ( $post_body, $url, $port ) {
  ...
  */
  ...
  if ( $response_string == FALSE ) {
    ...
    $sms_result['details'] .= "$response_string\n";
    ...
    if ( count( $api_result ) != 3 ) {
      ...
      if ($status_code == '0') {
        ...
        $sms_result['details'] .= "[$api_result[0]] : [$api_result[1]]\n";
      }
    }
  }
  curl_close( $ch );
